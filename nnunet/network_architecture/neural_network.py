...
class SegmentationNetwork(NeuralNetwork):
    ...
    @staticmethod
    def _compute_steps_for_sliding_window(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float, patchSparse) -> List[List[int]]:
        ...
        if patchSparse == 0:
            ...
            #  决定 y 轴的steps，step个数固定为3
            if image_size[1] > 1.5 * patch_size[1]:
                steps_here = [int(np.round(image_size[1] / 2) - 0.75 * patch_size[1]),
                              int(np.round(image_size[1] / 2) - 0.5 * patch_size[1]),
                              int(np.round(image_size[1] / 2) - 0.25 * patch_size[1])]
            elif image_size[1] > patch_size[1]:
                actual_step_size = (image_size[1] - patch_size[1]) / 1
                steps_here = [int(np.round(0)),
                              int(np.round(image_size[1] / 2) - 0.5 * patch_size[1]),
                              int(np.round(actual_step_size))]
            else:
                steps_here = [0, 0, 0]
            steps.append(steps_here)
            ...
            # 准备 x 轴的 steps，step个数固定为3
            if image_size[2] > 1.5 * patch_size[2]:
                steps_here = [int(np.round(image_size[2] / 2) - 0.75 * patch_size[2]),
                              int(np.round(image_size[2] / 2) - 0.5 * patch_size[2]),
                              int(np.round(image_size[2] / 2) - 0.25 * patch_size[2])]
            elif image_size[2] > patch_size[2]:
                actual_step_size = (image_size[2] - patch_size[2]) / 1
                steps_here = [int(np.round(0)),
                              int(np.round(image_size[2] / 2) - 0.5 * patch_size[2]),
                              int(np.round(actual_step_size))]
            else:
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        patchSparse = 0
        steps = self._compute_steps_for_sliding_window(patch_size, data_shape[1:], step_size, patchSparse)
        ...
        if num_tiles>=50 :
            patchSparse = 1
            steps = self._compute_steps_for_sliding_window(patch_size, data_shape[1:], step_size, patchSparse)
            num_tiles = len(steps[0]) * ( (len(steps[1])-1) * (len(steps[2])-1) + 1 )


        if verbose:
            print("data shape:", data_shape)
            print("patch size:", patch_size)
            print("steps (z, y, and x):", steps)
            print("number of tiles:", num_tiles)
            print("patchSparse:", patchSparse)
        ...
        if patchSparse == 0:
            ...
            for z in steps[0]:
                ...
                # 对该 xy 平面共有 5 个 window， 对正中央的 1 个 window 进行推理
                lb_z = z
                ub_z = z + patch_size[0]
                ...
                num_window += 1
                is_empty = torch.sum(torch.argmax(predicted_patch, 0) > 0) < 1000
                is_half_empty = torch.sum(
                    torch.argmax(predicted_patch[:, :int(0.6 * patch_size[1]), :, :], 0) > 0) < 500

                if not (is_empty or is_half_empty):
                    labels = torch.unique(torch.argmax(predicted_patch, 0))
                    is_tube = torch.logical_or(
                        torch.logical_or(torch.logical_or(labels == 0, labels == 5), labels == 10), labels == 6).all()
                    if is_tube:
                        tube_front = torch.sum(torch.argmax(predicted_patch[:, :, 0, :], 0) > 0) > 10
                        tube_back = torch.sum(torch.argmax(predicted_patch[:, :, -1, :], 0) > 0) > 10
                    if all_in_gpu:
                    ...
                    for y in steps[1]:
                        ...
                        for x in steps[2]:
                            ...
                            if y == steps[1][1] or x == steps[2][1]:  # 中间列不用再跑了
                                continue
